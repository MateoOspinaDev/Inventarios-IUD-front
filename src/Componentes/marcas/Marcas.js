...
import { obtenerMarcas,crearMarca,editarMarcaPorID,borrarMarcaPorID } from '../../services/MarcaService'
...
export default function Marcas() {
  ...
  const guardarMarca = async () => {
    ...
    setLoading(true)
    try{
      const res = await crearMarca(estado)
      console.log(res)
      setLoading(true)
      setEstado({nombre: ''})
      listMarcas()
  ...
  const editarMarca = async (e) => {
    ...
    try{
      setError(false)
      const resp = await editarMarcaPorID(estado._id, estado);
      console.log(resp)
      resetEstado()
      listMarcas()
    }catch(e){
  ...
  const setEstadoPorId = (e) => {
    ...
    const estadosFilter = marcas.filter(t => t._id == e.target.id);
